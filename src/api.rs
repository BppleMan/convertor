...
pub mod sub_provider_api;

pub enum UniversalProviderApi {
    BosLife(BosLifeApi),
}

impl UniversalProviderApi {
    pub fn get_service_provider_api(config: SubProviderConfig, base_dir: impl AsRef<Path>) -> UniversalProviderApi {
        match config.provider {
            SubProvider::BosLife => UniversalProviderApi::BosLife(BosLifeApi::new(base_dir, config)),
        }
    }

    pub fn client(&self) -> &ReqwestClient {
        match self {
            UniversalProviderApi::BosLife(api) => api.client(),
        }
    }
    ...
    pub async fn get_raw_sub_url(&self) -> color_eyre::Result<Url> {
        SubProviderApi::get_raw_sub_url(self).await
    }

    pub async fn reset_raw_sub_url(&self) -> color_eyre::Result<Url> {
        SubProviderApi::reset_raw_sub_url(self).await
    }
...
impl SubProviderApi for UniversalProviderApi {
    fn config(&self) -> &SubProviderConfig {
        match self {
            UniversalProviderApi::BosLife(api) => api.config(),
        }
    }
    ...
    fn login_request(&self) -> color_eyre::Result<Request> {
        match self {
            UniversalProviderApi::BosLife(api) => api.login_request(),
        }
    }

    fn get_sub_request(&self, auth_token: impl AsRef<str>) -> color_eyre::Result<Request> {
        match self {
            UniversalProviderApi::BosLife(api) => api.get_sub_request(auth_token),
        }
    }

    fn reset_sub_request(&self, auth_token: impl AsRef<str>) -> color_eyre::Result<Request> {
        match self {
            UniversalProviderApi::BosLife(api) => api.reset_sub_request(auth_token),
        }
    }

    fn get_sub_logs_request(&self, auth_token: impl AsRef<str>) -> color_eyre::Result<Request> {
        match self {
            UniversalProviderApi::BosLife(api) => api.get_sub_logs_request(auth_token),
        }
    }

    fn cached_auth_token(&self) -> &MokaCache<String, String> {
        match self {
            UniversalProviderApi::BosLife(api) => api.cached_auth_token(),
        }
    }

    fn cached_profile(&self) -> &Cache<Url, String> {
        match self {
            UniversalProviderApi::BosLife(api) => api.cached_profile(),
        }
    }

    fn cached_raw_sub_url(&self) -> &Cache<Url, String> {
        match self {
            UniversalProviderApi::BosLife(api) => api.cached_raw_sub_url(),
        }
    }

    fn cached_sub_logs(&self) -> &Cache<Url, BosLifeSubLogs> {
        match self {
            UniversalProviderApi::BosLife(api) => api.cached_sub_logs(),
        }
    }
}
