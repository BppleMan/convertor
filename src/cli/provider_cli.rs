    ...
    pub async fn execute(&mut self, cmd: ProviderCmd) -> Result<(UrlBuilder, ProviderCliResult)> {
        ...
        if cmd.update {
            match _client_profile {
                ClientProfile::Surge => {
                    super::update_local::update_surge_config(&self.config, &url_builder, &policies).await?;
                }
                ClientProfile::Clash(profile) => {
                    super::update_local::update_clash_config(&self.config, &url_builder, profile).await?;
                }
            }
        }
