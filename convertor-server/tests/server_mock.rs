...
const SURGE_MOCK_DIR: Dir = include_dir!("$CARGO_MANIFEST_DIR/.convertor/surge");
const CLASH_MOCK_DIR: Dir = include_dir!("$CARGO_MANIFEST_DIR/.convertor/clash");

static INITIALIZED_TEST: Once = Once::new();

pub fn init_test_base_dir() -> PathBuf {
    std::env::current_dir().unwrap().join(".convertor")
}
